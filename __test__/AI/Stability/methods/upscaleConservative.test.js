import upscaleConservative from '../../../../lib/connectors/AI/Stability/methods/upscaleConservative';
...
describe("Stability - upscaleConservative method", () => {
    let httpRequestMock;
    let throwErrorMock;
    // test variables
    ...
    describe('API Interaction', () => {
        it('Test make a POST request to the correct endpoint', async () => {
            ...
            const response = await upscaleConservative(httpRequestMock, throwErrorMock, prompt, pathImage, destinationFolder, output_format, config);
    ...
    describe("Throw the AIConnectifyError error", () => {
        it('Test when prompt is not provided', async () => {
            await expect(
                upscaleConservative(httpRequestMock, throwErrorMock, undefined)
            ).rejects.toThrow('Cannot process the prompt');
        });
        it('Test when prompt is empty string', async () => {
            await expect(
                upscaleConservative(httpRequestMock, throwErrorMock, '')
            ).rejects.toThrow('Cannot process the prompt');
        });
        it('Test when prompt is not a string', async () => {
            await expect(
                upscaleConservative(httpRequestMock, throwErrorMock, 123)
            ).rejects.toThrow('Cannot process the prompt');
        });

        it('Test when pathImage is not provided', async () => {
            await expect(
                upscaleConservative(httpRequestMock, throwErrorMock, prompt, undefined)
            ).rejects.toThrow('Cannot process the image path folder');
        });
        it('Test when pathImage is empty string', async () => {
            await expect(
                upscaleConservative(httpRequestMock, throwErrorMock, prompt, '')
            ).rejects.toThrow('Cannot process the image path folder');
        });
        it('Test when pathImage is not a string', async () => {
            await expect(
                upscaleConservative(httpRequestMock, throwErrorMock, prompt, 123)
            ).rejects.toThrow('Cannot process the image path folder');
        });

        it('Test when destinationFolder is not provided', async () => {
            await expect(
                upscaleConservative(httpRequestMock, throwErrorMock, prompt, pathImage, undefined, output_format)
            ).rejects.toThrow('Cannot process the destination folder');
        });
        it('Test when destinationFolder is empty string', async () => {
            await expect(
                upscaleConservative(httpRequestMock, throwErrorMock, prompt, pathImage, '', output_format)
            ).rejects.toThrow('Cannot process the destination folder');
        });
        it('Test when destinationFolder is not a string', async () => {
            await expect(
                upscaleConservative(httpRequestMock, throwErrorMock, prompt, pathImage, 123, output_format)
            ).rejects.toThrow('Cannot process the destination folder');
        });

        it('Test when output_format is not provided', async () => {
            await expect(
                upscaleConservative(httpRequestMock, throwErrorMock, prompt, pathImage, destinationFolder, undefined)
            ).rejects.toThrow('Cannot process the output format');
        });
        it('Test when output_format is empty string', async () => {
            await expect(
                upscaleConservative(httpRequestMock, throwErrorMock, prompt, pathImage, destinationFolder, '')
            ).rejects.toThrow('Cannot process the output format');
        });
        it('Test when output_format is not a string', async () => {
            await expect(
                upscaleConservative(httpRequestMock, throwErrorMock, prompt, pathImage, destinationFolder, 123)
            ).rejects.toThrow('Cannot process the output format');
        });
        ...
        it('Test handle API errors correctly', async () => {
            ...
            await upscaleConservative(httpRequestMock, throwErrorMock, prompt, pathImage, destinationFolder, output_format, config);
