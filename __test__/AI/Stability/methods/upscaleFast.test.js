import upscaleFast from '../../../../lib/connectors/AI/Stability/methods/upscaleFast';
...
describe("Stability - upscaleFast method", () => {
    let httpRequestMock;
    let throwErrorMock;
    // test variables
    ...
    describe('API Interaction', () => {
        it('Test make a POST request to the correct endpoint', async () => {
            ...
            const response = await upscaleFast(httpRequestMock, throwErrorMock, pathImage, destinationFolder, output_format);
    ...
    describe("Throw the AIConnectifyError error", () => {
        it('Test when pathImage is not provided', async () => {
            await expect(
                upscaleFast(httpRequestMock, throwErrorMock, undefined)
            ).rejects.toThrow('Cannot process the image path folder');
        });
        it('Test when pathImage is empty string', async () => {
            await expect(
                upscaleFast(httpRequestMock, throwErrorMock, '')
            ).rejects.toThrow('Cannot process the image path folder');
        });
        it('Test when pathImage is not a string', async () => {
            await expect(
                upscaleFast(httpRequestMock, throwErrorMock, 123)
            ).rejects.toThrow('Cannot process the image path folder');
        });

        it('Test when destinationFolder is not provided', async () => {
            await expect(
                upscaleFast(httpRequestMock, throwErrorMock, pathImage, undefined, output_format)
            ).rejects.toThrow('Cannot process the destination folder');
        });
        it('Test when destinationFolder is empty string', async () => {
            await expect(
                upscaleFast(httpRequestMock, throwErrorMock, pathImage, '', output_format)
            ).rejects.toThrow('Cannot process the destination folder');
        });
        it('Test when destinationFolder is not a string', async () => {
            await expect(
                upscaleFast(httpRequestMock, throwErrorMock, pathImage, 123, output_format)
            ).rejects.toThrow('Cannot process the destination folder');
        });

        it('Test when output_format is not provided', async () => {
            await expect(
                upscaleFast(httpRequestMock, throwErrorMock, pathImage, destinationFolder, undefined)
            ).rejects.toThrow('Cannot process the output format');
        });
        it('Test when output_format is empty string', async () => {
            await expect(
                upscaleFast(httpRequestMock, throwErrorMock, pathImage, destinationFolder, '')
            ).rejects.toThrow('Cannot process the output format');
        });
        it('Test when output_format is not a string', async () => {
            await expect(
                upscaleFast(httpRequestMock, throwErrorMock, pathImage, destinationFolder, 123)
            ).rejects.toThrow('Cannot process the output format');
        });
        ...
        it('Test handle API errors correctly', async () => {
            ...
            await upscaleFast(httpRequestMock, throwErrorMock, pathImage, destinationFolder, output_format);
