import generateImageCore from '../../../../lib/connectors/AI/Stability/methods/generateImageCore';
...
describe("Stability - generateImageCore method", () => {
    let httpRequestMock;
    let throwErrorMock;
    // test variables
    ...
    describe('API Interaction', () => {
        it('Test make a POST request to the correct endpoint', async () => {
            ...
            const response = await generateImageCore(httpRequestMock, throwErrorMock, prompt, destinationFolder, output_format, config);
    ...
    describe("Throw the AIConnectifyError error", () => {
        it('Test when prompt is not provided', async () => {
            await expect(
                generateImageCore(httpRequestMock, throwErrorMock, undefined)
            ).rejects.toThrow('Cannot process the prompt');
        });
        it('Test when prompt is empty string', async () => {
            await expect(
                generateImageCore(httpRequestMock, throwErrorMock, '')
            ).rejects.toThrow('Cannot process the prompt');
        });
        it('Test when prompt is not a string', async () => {
            await expect(
                generateImageCore(httpRequestMock, throwErrorMock, 123)
            ).rejects.toThrow('Cannot process the prompt');
        });

        it('Test when destinationFolder is not provided', async () => {
            await expect(
                generateImageCore(httpRequestMock, throwErrorMock, prompt, undefined, output_format)
            ).rejects.toThrow('Cannot process the destination folder');
        });
        it('Test when destinationFolder is empty string', async () => {
            await expect(
                generateImageCore(httpRequestMock, throwErrorMock, prompt, '', output_format)
            ).rejects.toThrow('Cannot process the destination folder');
        });
        it('Test when destinationFolder is not a string', async () => {
            await expect(
                generateImageCore(httpRequestMock, throwErrorMock, prompt, 123, output_format)
            ).rejects.toThrow('Cannot process the destination folder');
        });
        
        it('Test when output_format is not provided', async () => {
            await expect(
                generateImageCore(httpRequestMock, throwErrorMock, prompt, destinationFolder, undefined)
            ).rejects.toThrow('Cannot process the output format');
        });
        it('Test when output_format is empty string', async () => {
            await expect(
                generateImageCore(httpRequestMock, throwErrorMock, prompt, destinationFolder, '')
            ).rejects.toThrow('Cannot process the output format');
        });
        it('Test when output_format is not a string', async () => {
            await expect(
                generateImageCore(httpRequestMock, throwErrorMock, prompt, destinationFolder, 123)
            ).rejects.toThrow('Cannot process the output format');
        });
        ...
        it('Test handle API errors correctly', async () => {
            ...
            await generateImageCore(httpRequestMock, throwErrorMock, prompt, destinationFolder, output_format, config);
