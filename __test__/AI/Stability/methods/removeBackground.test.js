import removeBackground from '../../../../lib/connectors/AI/Stability/methods/removeBackground';
...
describe("Stability - removeBackground method", () => {
    let httpRequestMock;
    let throwErrorMock;
    // test variables
    ...
    describe('API Interaction', () => {
        it('Test make a POST request to the correct endpoint', async () => {
            ...
            const response = await removeBackground(httpRequestMock, throwErrorMock, pathImage, destinationFolder, output_format);
    ...
    describe("Throw the AIConnectifyError error", () => {
        it('Test when pathImage is not provided', async () => {
            await expect(
                removeBackground(httpRequestMock, throwErrorMock, undefined)
            ).rejects.toThrow('Cannot process the image path folder');
        });
        it('Test when pathImage is empty string', async () => {
            await expect(
                removeBackground(httpRequestMock, throwErrorMock, '')
            ).rejects.toThrow('Cannot process the image path folder');
        });
        it('Test when pathImage is not a string', async () => {
            await expect(
                removeBackground(httpRequestMock, throwErrorMock, 123)
            ).rejects.toThrow('Cannot process the image path folder');
        });

        it('Test when destinationFolder is not provided', async () => {
            await expect(
                removeBackground(httpRequestMock, throwErrorMock, pathImage, undefined, output_format)
            ).rejects.toThrow('Cannot process the destination folder');
        });
        it('Test when destinationFolder is empty string', async () => {
            await expect(
                removeBackground(httpRequestMock, throwErrorMock, pathImage, '', output_format)
            ).rejects.toThrow('Cannot process the destination folder');
        });
        it('Test when destinationFolder is not a string', async () => {
            await expect(
                removeBackground(httpRequestMock, throwErrorMock, pathImage, 123, output_format)
            ).rejects.toThrow('Cannot process the destination folder');
        });

        it('Test when output_format is not provided', async () => {
            await expect(
                removeBackground(httpRequestMock, throwErrorMock, pathImage, destinationFolder, undefined)
            ).rejects.toThrow('Cannot process the output format');
        });
        it('Test when output_format is empty string', async () => {
            await expect(
                removeBackground(httpRequestMock, throwErrorMock, pathImage, destinationFolder, '')
            ).rejects.toThrow('Cannot process the output format');
        });
        it('Test when output_format is not a string', async () => {
            await expect(
                removeBackground(httpRequestMock, throwErrorMock, pathImage, destinationFolder, 123)
            ).rejects.toThrow('Cannot process the output format');
        });
        ...
        it('Test handle API errors correctly', async () => {
            ...
            await removeBackground(httpRequestMock, throwErrorMock, pathImage, destinationFolder, output_format);
