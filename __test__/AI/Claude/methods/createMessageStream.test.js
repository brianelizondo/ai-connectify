import createMessageStream from '../../../../lib/connectors/AI/Claude/methods/createMessageStream';

describe("Claude - createMessageStream method", () => {
    let httpRequestMock;
    let throwErrorMock;
    // test variables
    const messagesArray = [
    ...
    describe('API Interaction', () => {
        it('Test make a POST request to the correct endpoint', async () => {
            ...
            expect(httpRequestMock.post).toHaveBeenCalledWith(
                '/messages', 
                { ...config, messages: messagesArray, model: modelId, max_tokens: maxTokens, stream: true }
            );
    ...
    describe("Throw the AIConnectifyError error", () => {
        it('Test when messagesArray is not provided', async () => {
            await expect(
                createMessageStream(httpRequestMock, throwErrorMock, undefined)
            ).rejects.toThrow('Cannot process the messages array');
        });
        it('Test when messagesArray is empty array', async () => {
            await expect(
                createMessageStream(httpRequestMock, throwErrorMock, [])
            ).rejects.toThrow('Cannot process the messages array');
        });
        it('Test when messagesArray is not a array', async () => {
            await expect(
                createMessageStream(httpRequestMock, throwErrorMock, 123)
            ).rejects.toThrow('This is not an array');
        });

        it('Test when modelID is not provided', async () => {
            await expect(
                createMessageStream(httpRequestMock, throwErrorMock, messagesArray, undefined)
            ).rejects.toThrow('Cannot process the model ID');
        });
        it('Test when modelID is empty string', async () => {
            await expect(
                createMessageStream(httpRequestMock, throwErrorMock, messagesArray, '')
            ).rejects.toThrow('Cannot process the model ID');
        });
        it('Test when modelID is not a string', async () => {
            await expect(
                createMessageStream(httpRequestMock, throwErrorMock, messagesArray, 123)
            ).rejects.toThrow('Cannot process the model ID');
        });

        it('Test when maxTokens is not provided', async () => {
            await expect(
                createMessageStream(httpRequestMock, throwErrorMock, messagesArray, modelId, undefined)
            ).rejects.toThrow('Cannot process the max tokens value');
        });
        it('Test when maxTokens is not a number', async () => {
            await expect(
                createMessageStream(httpRequestMock, throwErrorMock, messagesArray, modelId, '123')
            ).rejects.toThrow('Cannot process the max tokens value');
        });
        ...
        it('Test handle API errors correctly', async () => {
            ...
            await createMessageStream(httpRequestMock, throwErrorMock, messagesArray, modelId, maxTokens, config);
