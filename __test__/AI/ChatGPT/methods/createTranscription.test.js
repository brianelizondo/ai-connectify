import createTranscription from '../../../../lib/connectors/AI/ChatGPT/methods/createTranscription';
import FormData from 'form-data';

jest.mock('form-data');

describe("ChatGPT - createTranscription method", () => {
    let httpRequestMock;
    let throwErrorMock;
    // test variables
    ...
    describe('API Interaction', () => {
        it('Test make a POST request to the correct endpoint', async () => {
            ...
            const response = await createTranscription(httpRequestMock, throwErrorMock, filePath, modelId, config);
    ...
    describe("Throw the AIConnectifyError error", () => {
        it('Test when filePath is not provided', async () => {
            await expect(
                createTranscription(httpRequestMock, throwErrorMock, undefined)
            ).rejects.toThrow('Cannot process the file path');
        });
        it('Test when filePath is empty string', async () => {
            await expect(
                createTranscription(httpRequestMock, throwErrorMock, '')
            ).rejects.toThrow('Cannot process the file path');
        });
        it('Test when filePath is not a string', async () => {
            await expect(
                createTranscription(httpRequestMock, throwErrorMock, 123)
            ).rejects.toThrow('Cannot process the file path');
        });
        it('Test when modelID is not provided', async () => {
            await expect(
                createTranscription(httpRequestMock, throwErrorMock, filePath, undefined)
            ).rejects.toThrow('Cannot process the model ID');
        });
        it('Test when modelID is empty string', async () => {
            await expect(
                createTranscription(httpRequestMock, throwErrorMock, filePath, '')
            ).rejects.toThrow('Cannot process the model ID');
        });
        it('Test when modelID is not a string', async () => {
            await expect(
                createTranscription(httpRequestMock, throwErrorMock, filePath, 123)
            ).rejects.toThrow('Cannot process the model ID');
        });
        ...
        it('Test handle API errors correctly', async () => {
            ...
            await createTranscription(httpRequestMock, throwErrorMock, filePath, modelId, config);
