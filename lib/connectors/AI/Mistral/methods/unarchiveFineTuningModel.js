...
*/
async function unarchiveFineTuningModel(httpRequest, throwError, fine_tuning_model_id){
    HelperFunctions.validateStringInput(fine_tuning_model_id, 'Cannot process the fine tunning model ID');

    try {
        const response = await httpRequest.delete(`/fine_tuning/models/${fine_tuning_model_id}/archive`);
        return response;
    } catch (error) {
        throwError(error);
    }
...
module.exports = unarchiveFineTuningModel;