...
*/
async function createMessageStream(httpRequest, throwError, messagesArray, modelID, maxTokens, newConfig) {
    ...
    try {
        const response = await httpRequest.post('/messages', { ...newConfig, messages: messagesArray, model: modelID, max_tokens: maxTokens, stream: true });
        return response;
    } catch (error) {
        throwError(error);
    }
